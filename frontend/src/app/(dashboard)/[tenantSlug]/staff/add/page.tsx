...
import { Camera, ArrowLeft, Plus, Eye, Trash2, Upload } from 'lucide-react';
  ...
  useEffect(() => {
    const initPage = async () => {
      try {
        ...
        console.error("Failed to initialize add staff page", err);
      }
    };
    initPage();
  }, []);
   ...
   const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (file.size > 5 * 1024 * 1024) { // Increased to 5MB for cropping flexibility
  ...
  const handleCropComplete = (croppedBlob: Blob) => {
   ...
   const validate = () => {
    ...
    if (formData.joining_date) {
      ...
      if (date < minDate) {
        newErrors.joining_date = "Date cannot be before 2020";
      } else if (date > maxDate) {
        newErrors.joining_date = "Date cannot be more than 1 year in future";
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSubmitting(true);
     try {
      ...
      const savedStaffId = res.data.staff_id;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center mb-8 gap-4">
        ...
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Add Staff Member</h1>
          <p className="text-slate-500 mt-1">Create a new staff profile and assign login credentials</p>
        </div>
      </div>
          ...
          <div className="p-8 space-y-8">
            ...
            <div className="bg-[#FAF9F6] border border-slate-100 rounded-2xl p-6 flex items-center gap-10">
              <div className="relative">
                <div className="w-28 h-28 rounded-full bg-white border-2 border-white shadow-md flex items-center justify-center relative overflow-hidden group">
                  <img 
                    src={previewUrl || `https://api.dicebear.com/7.x/notionists/svg?seed=new`} 
                    alt="Avatar Preview" 
            ...
            <div className="grid grid-cols-2 gap-6 text-sm">
              <div className="space-y-1.5">
                <label className="text-slate-600 font-medium pb-2 flex items-center">
                  Staff ID <span className={`ml-2 px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-wider ${generationMode === 'Automatic' ? 'bg-[#C69A70]/10 text-[#C69A70]' : 'bg-blue-100 text-blue-600'}`}>
                    {generationMode === 'Automatic' ? 'Auto' : 'Manual'}
                  </span>
                </label>
                <input 
                  type="text" 
                  ...
                  maxLength={20}
                  placeholder={generationMode === 'Manual' ? "Enter Staff ID" : ""}
                  className={`w-full px-4 py-3 rounded-xl border font-bold shadow-sm transition-all ${errors.staff_code ? 'border-red-400 bg-red-50' : 'border-slate-200'} ${generationMode === 'Automatic' ? 'bg-slate-50 text-slate-500 cursor-not-allowed border-dashed border-slate-200' : 'bg-white text-slate-900 border-slate-200 focus:ring-2 focus:ring-[#C69A70]'}`}
                />
                {errors.staff_code && generationMode === 'Manual' && <p className="text-red-500 text-[10px] mt-1 font-bold absolute -bottom-5 left-1">{errors.staff_code}</p>}
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-slate-600 font-medium">Name</label>
                 <input 
                  ...
                  minLength={2}
                  maxLength={100}
                  className={`w-full px-4 py-3 rounded-xl border ${errors.name ? 'border-red-400 bg-red-50' : 'border-slate-200'} focus:outline-none focus:ring-2 focus:ring-[#C69A70] focus:border-transparent transition-all shadow-sm`}
                />
                {errors.name && <p className="text-red-500 text-xs mt-1 absolute -bottom-5 left-0">{errors.name}</p>}
              </div>
              
              <div className="space-y-1.5 relative">
                <label className="text-slate-600 font-medium bg-blue-500/10 text-blue-600 px-1 rounded inline-block">Email</label>
                <input 
                  type="text" 
                  required
                  ...
                  maxLength={254}
                  className={`w-full px-4 py-3 rounded-xl border ${errors.email ? 'border-red-400 bg-red-50' : 'border-slate-200 bg-blue-50/30'} focus:outline-none focus:ring-2 focus:ring-[#C69A70] focus:border-transparent transition-all shadow-sm`}
                />
              ...
              <div className="space-y-1.5 relative mt-2">
                <label className="text-slate-600 font-medium">Password (for login)</label>
                <input 
                  type="password" 
                  required
                  ...
                  minLength={8}
                  maxLength={64}
                  className={`w-full px-4 py-3 rounded-xl border ${errors.password ? 'border-red-400' : 'border-slate-200'} focus:outline-none focus:ring-2 focus:ring-[#C69A70] focus:border-transparent transition-all shadow-sm`}
                />
              ...
              <div className="space-y-1.5 relative mt-2">
                ...
                <input 
                  ...
                  maxLength={20}
                  className={`w-full px-4 py-3 rounded-xl border ${errors.whatsapp_number ? 'border-red-400' : 'border-slate-200'} focus:outline-none focus:ring-2 focus:ring-[#C69A70] focus:border-transparent transition-all shadow-sm`}
                />
              ...
              <div className="space-y-1.5 mt-2">
                ...
                <select 
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#C69A70] border-transparent transition-all bg-white shadow-sm"
                >
                  <option value="">No Store / Team </option>
                  {stores
                    .filter(store => 
                      formData.role === 'Staff' || 
                      !store.manager_name || 
              ...
              <div className="space-y-1.5 mt-2">
                <label className="text-slate-600 font-medium">Joining Date</label>
                 <input 
                  ...
                  className={`w-full px-4 py-3 rounded-xl border ${errors.joining_date ? 'border-red-400 bg-red-50' : 'border-slate-200'} focus:outline-none focus:ring-2 focus:ring-[#C69A70] border-transparent transition-all bg-white shadow-sm relative`}
                />
                {errors.joining_date && <p className="text-red-500 text-xs mt-1 absolute -bottom-5 left-0">{errors.joining_date}</p>}
              </div>
